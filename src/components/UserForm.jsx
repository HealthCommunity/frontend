...
function UserForm({ isJoin = false, onSubmit }) {
    ...
    const checkValidityId = (id) => {
        if (id.length < 5) {
            return [false, "id가 너무 짧습니다. 아이디는 5자 이상 12자 이하로 입력해주세요."]
        } else if (id.length > 12) {
            return [false, "id가 너무 깁니다. 아이디는 5자 이상 12자 이하로 입력해주세요."]
        } else {
            if (isRuleId(id)) {
    ...
    const checkValidityPassword = (password) => {
        if (password.length < 8) {
            return [false, "비밀번호가 너무 짧습니다. 8자 이상 16자 이하 숫자, 문자, 특수문자를 입력해주세요."]
        } else if (password.length > 16) {
            return [false, "비밀번호가 너무 깁니다. 8자 이상 15자 이하 숫자, 문자, 특수문자를 입력해주세요."]
        } else {
    ...
    const checkValidityNickname = (nickname) => {
        if (nickname.length < 2) {
            return [false, "닉네임이 너무 짧습니다. 2자 이상 10자 이하 문자를 입력해주세요."]
        } else if (nickname.length > 10) {
            return [false, "닉네임이 너무 깁니다. 2자 이상 10자 이하 문자를 입력해주세요."]
        } else {
    ...
    const handleSubmit = (e) => {
        ...
        if (!isJoin) {
            return onSubmit(e, { id: usrInputs.id, password: usrInputs.password })
        }
            ...
            {isJoin && <> <InputGroup>
                ...
                <InputGroup>
                    <CheckInput>
                        ...
                        <Label
                            htmlFor="agree">이용약관 및 개인정보 수집∙이용에 동의합니다.
                        </Label>
                    </CheckInput>
                </InputGroup>
            </>}
...
function TextInput({ defaultValue = "", onChange, onValidation, ...props }) {
    ...
    return <>
        ...
        {!!msg && <span style={{ color: isSuccess ? "tomato" : "blue" }}>{msg}</span>}
    </>
}
